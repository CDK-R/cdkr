...
In contrast, the `generate.formula.iter` method employs the Round Robin formula generation akgorithm, which is significantly faster than the default method. In contrast to `generate/formula`, this method returns an iterator which allows you to process large formula sets without excessive memory usage.
```{r warn=FALSE}
...
formula. `rcdk` provides the function `get.isotopexss.pattern` which predicts
...
pattern can be visually inspectd, as shown below.
```{r}
